...
| **norm** | `source_artifact` (intake provenance) |
...
- **Capacity (directly relevant to E-38):** `bid.capacity_statement` (per cycle/round/supplier/submission header — status, effective_at) + `bid.capacity_constraint` (`scope_type` ∈ CELL/DC_TF/LOT_TF/SUPPLIER_TF/TOTAL_CYCLE, with `max_weekly_cases` / `max_period_cases` + CHECKs) + `eng.scenario_capacity_usage`. **E-38 wires these existing tables** — it does NOT add a new capacity store.
...
- **Commercial / market / performance:** `perf.commercial_*` (pricing model/window/formula audit/market reference/QDP/…), `perf.historical_award_*`, `perf.itrade_receipt` (raw feed; importer is future E-08). `ref.fiscal_calendar`, `ref.subcommodity`, `ref.loading_location`, `ref.master_data_quarantine`. `norm.normalization_run(_source)`. `audit.decision_note`, `audit.round_*`.
...
| Status | Items |
|---|---|
| **Approved for Phase 1 build** | **E-38 capacity accuracy-core** (B: ingest + persist + engine/custom cap flag + workbook control tab) — **wires the EXISTING `bid.capacity_statement` + `bid.capacity_constraint` (+ `eng.scenario_capacity_usage`) baseline tables (§16), not a new store**; the only net-new build approved now |
