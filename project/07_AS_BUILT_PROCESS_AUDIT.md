...
| Web console (UI) | 🟡 Partial — dashboard, intake, **alignment/scenario/freeze**, and **post-award award view (read)** wired; the adjustment *write*, sign-off, close-out still MCP-only (G-E) |
...
| **G-E** | The HTTP API was front-half only. Now wired end to end: `run_round` + scenario reads + `freeze_award` (alignment screen), and the **frozen-award read** (`awd/read.py`: list awards, award detail with baseline + effective $/cell + version history) with a web **Awards screen**. Still MCP-only: **recording** a `record_adjustment` (the post-award *write*); the `documents` router is an empty stub. | 🟠 Material | The console can run the engine, compare scenarios, freeze, and **view the frozen award + its layer history** from the browser; recording an adjustment + document send still need the MCP harness. | E-25 — remaining: the adjustment *write* endpoint + screen, and the document HTTP surface. | 🟡 Partial (alignment + award read shipped; adjustment write open) |
...
| 8 | Post-award adjustments | `record_adjustment` (service.py:518) → `awd.award_adjustment(_line)` | V·S | read: `GET /runs/{slug}/awards/{id}` · write: **MCP only** ⛔ | Buyer → **Awards screen ◐** → views the layers; recording an adjustment is still MCP-only |
...
- HTTP API: engine/scenario/freeze + the **frozen-award read** now wired (HTTP + alignment/awards screens); **recording** a `record_adjustment` (the post-award *write*) + the `documents` router still MCP-only/empty → **E-25 (remainder)**.
...
2. **Alignment screen ✅ (v1.8); post-award award *read* ✅ (v1.9)** (G-E, E-25) — run/compare/inspect/freeze + view the frozen award (baseline + effective $/cell + version history) from the browser. **Remaining for G-E:** the post-award **adjustment *write*** (record a layer) + the **document** HTTP surface + screens → **← next**.
