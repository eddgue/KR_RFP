...
| **Finalize / close-out (console)** | `finalize_run` (service.py:1321) — GATE: a FROZEN award for the run's cycle is required (else 409 conflict); emits **A: CLOSED** (entity = `cyc.cycle`, actor = authenticated user) in-txn via `AuditWriter`; surfaces the closing deliverables (FROZEN award id + won/not-won supplier counts) from the existing `award_email_drafts`/`rejection_email_drafts` generators. **Idempotent** (a 2nd finalize is a clean no-op — same summary, no 2nd CLOSED). NOT a delete (run + records remain); writes **no file** (E-42). Closed state is **derived** from the CLOSED event (`is_closed` service.py:1390 — no column/migration). | S·A (console) | HTTP `POST …/finalize` (runs.py:619) | Buyer → **Close out the run** (design "Finalize & close run") → assert close-out (actor recorded). **Front-end screen ◐ (design only).** |
| Close-out (harness teardown) | `close_run` (service.py:1287) → archive zip; `purge_run` (service.py:1297) → drop run DB | V (harness) | **MCP only** ⛔ `close_run` (:501) / `purge_run` (:518) | Buyer → **Close-out screen ⬜** |
...
| Audit (decision events, in-txn) | `service.py:1602` (SUPERSEDED), `:1671` (IMPORTED), `:706` (SEALED); `awd/service.py:158` (FROZEN), `:247` (CREATED); **`service.py:1374` (CLOSED — run finalize, entity = `cyc.cycle`)**; writer `core/audit/writer.py:134` | `audit.event_log` | per-tenant `client_id`+`seq` (`FOR UPDATE`); tenant resolved cycle/award→commodity→client (`recorder.py:20,40`); **raises if unresolvable**. CLOSED is the run's terminal close-out + the **derived** closed-state source (no `pilot.run` column) |
...
There is **no optimisation loop inside the engine** — `run_analysis` is single-pass, deterministic, with hashed input/output manifests (runner.py:150-155). The **input manifest seals the FULL frozen config** (`model_dump`, C2) — every weight/preset/threshold/premium-band/safety + exclusions/custom-overrides/preferred-rules/per-lot-thresholds — so a post-seal change to ANY strategy input is tamper-evident, not just the headline settings.

## 8. Audit / event-log status (G-B detail)

Mechanics (`core/audit/writer.py`): `prev_event_hash → event_hash = sha256(canonical(fields) ‖ prev)` (`compute_event_hash:46-79`), per-tenant `seq` `FOR UPDATE` (:95-104), genesis = 64 zeros, appended in the caller's txn — **no internal commit** (:106-111). Tenant resolved cycle→commodity→`client_id` / award→cycle→commodity (`recorder.py:20,40`); unresolvable tenant **raises**. **9 EventTypes defined** (events.py:20-28).
...
| `CLOSED` | service.py:1374 (in `finalize_run`) | ✅ | run finalize / close-out; entity = `cyc.cycle`; actor = authenticated user; the **derived** closed-state source; idempotent (no 2nd emission) |
...
| 21 | `POST /runs/{slug}/finalize` | runs.py:619 | CurrentUser (**no perm**) | gate: frozen award required else 409 conflict; 404 if no run | finalize / close-out (service.py:1321); **CLOSED** in-txn (entity = `cyc.cycle`, actor = `user.username`); idempotent; surfaces won/not-won notice counts; **no file, no delete** (E-42) |
| 22 | `GET /runs/{slug}/awards` | runs.py:642 | CurrentUser | — | list frozen awards |
| 23 | `GET /runs/{slug}/awards/{id}` | runs.py:663 | CurrentUser | `_has_cycle` else 404 | award detail: baseline + effective + history |
| 24 | `POST /runs/{slug}/awards/{id}/adjustments` | runs.py:696 | CurrentUser (**no perm**) | `RecordAdjustmentRequest`; off-award → 400; dup cell → 400; cross-run → 404 | append post-award layer (service.py:874); CREATED in-txn; post-award doc renders on request |
| 25 | `GET /runs/{slug}/awards/{id}/comms/award` | runs.py:782 | CurrentUser | `_has_cycle` else 404 | E-37 award drafts; **draft-only, no send, no DB write** |
| 26 | `GET /runs/{slug}/awards/{id}/comms/rejection` | runs.py:816 | CurrentUser | `_has_cycle` else 404 | E-37 non-selection drafts; draft-only |
| 27 | `GET /runs/{slug}/analysis/{id}/comms/feedback` | runs.py:851 | CurrentUser | `_ensure_analysis` | E-37 round-feedback drafts; draft-only |
| 28 | `POST /bids/import` | bids.py:176 | CurrentUser (**no FEED_IMPORT perm**) | mode∈{strict,flexible}, round≥1, confirm | **uploaded file streamed, never written to disk**: strict (`ingest_bids_bytes`, service.py:573) or flexible propose→confirm (`ingest_any_bytes`, service.py:607); IMPORTED+SUPERSEDED in-txn (actor = `user.username`); persists capacity (service.py:1734-1778) |
| 29 | `GET /bids` | bids.py:226 | CurrentUser | run, round≥1 | list a round's `bid.bid_line` at identity grain (ACTIVE only, DISTINCT ON) |

> **Mount-point note:** the analysis/award/finalize/adjustment/comms endpoints (#16–#27) live under the **`runs`** router, not the empty `awards` stub.
...
- **v1.26 (2026-06-22)** — *In-app finalize / close-out BUILT on the web console (C5, sponsor-approved; the design's "Finalize & close run" step).* After an award is FROZEN the buyer takes a terminal governed action that **locks the run CLOSED** and makes the **award notices (won) + rejection notices (not-won)** available — NOT a delete (the harness-only `close_run`/`purge_run` deletes; this is the governed close-out). **New EventType `CLOSED`** (events.py:28) — the design's run-finalize event. **`PilotService.finalize_run`** (service.py:1321): GATE = a FROZEN award for the run's cycle (else **409 conflict**, an un-awarded run is never finalized); emits one governed `CLOSED` `DomainEvent` in-txn via `AuditWriter` (entity = `cyc.cycle`, `entity_id`=cycle_id, actor = authenticated user, ids only); **idempotent** (a 2nd finalize is a clean no-op — same summary, no 2nd CLOSED). The award + rejection notices reuse the EXISTING render-on-request generators (`award_email_drafts`/`rejection_email_drafts`); finalize persists **no file** (ADR-0018 / E-42). **Closed state is DERIVED** from the CLOSED event existing for the cycle (`is_closed`/`_is_closed` service.py:1390-1404) — **no `finalized_at` column, no migration** (a derived state was genuinely sufficient: the tamper-evident event is the single source of truth and is queryable by `cycle_id`). **Endpoint** `POST /runs/{slug}/finalize` (runs.py:619, `FinalizeRunResponse`: closed / award_id / won_suppliers / not_won_suppliers); 404 if no run, 409 if not yet frozen. **§2/§3/§8/§9/§14 updated** (new finalize stage row distinguished from the harness teardown; CLOSED added to the audit write-points + event table; live-route count 28→29 / runs 19→20, endpoint table re-numbered; §8 EventType count 8→9). Partially addresses the design's finalize step (the notices it surfaces are draft-only — send is still G-D/E-24) and is adjacent to G-D sign-off. **The MCP harness is UNTOUCHED** (it keeps its own `close_run`/`purge_run` file-vault teardown). **6 new tests** (`tests/api/test_finalize.py`): finalize-after-freeze emits exactly one CLOSED + the notices are available + counts equal the rendered drafts; finalize-without-a-frozen-award is 409 (no CLOSED written); re-finalize is idempotent (no 2nd CLOSED); auth 401; unknown run 404; pre-setup 409. **257 pass** (251 baseline + 6); ruff/format/mypy clean. Promoted from triage C5.
- **v1.25 (2026-06-22)** — *Analysis tamper-seal tightened to cover ALL inputs (C2, sponsor-approved).* `runner._inputs_manifest` previously hand-listed a subset of config fields, so a post-seal change to an un-hashed field (premium bands, `single_supplier_per_lot`, exclusions, custom overrides, preferred rules, per-lot thresholds, active TFs) would NOT change the `input_hash`. Now the **full frozen `EngineConfig` is sealed** (`model_dump(mode="json")`), so any strategy-input change is tamper-evident and the seal can't drift when a config field is added. §8 reproducibility note updated; 4 new pure tests (`tests/engine/test_input_manifest.py`); 251 pass, no hash value was pinned so nothing else moved. Promoted from triage C2.
