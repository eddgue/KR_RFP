...
- **Pricing basis (D43) — part of the strategy panel:**
  ...
  - **Cost-line manager** — the price breakdown is **configurable**, not fixed: a list of cost lines the buyer can **toggle on/off** (e.g. **RPCs**) and **add to**. Each line shows: name · **add/subtract** · **grain** (per-period vs yearly, per D42) · **unit ($ /case or %)** · (if %) **a base = a buyer-selected set of ONE OR MANY other cost lines (a multi-select) + apply-order**. **Discounts** are just **subtract** lines: the buyer **picks which cost line(s) the discount applies to — one or many** (FOB default), and it reduces the sum of those. Changing the active set changes the template columns suppliers fill — surface that link.
