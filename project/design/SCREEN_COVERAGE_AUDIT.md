...
| **Editable column mapper (messy-file ingest)** | ◐ | Bid Intake has a *confirm-only* mapper: it shows the inferred column→field mapping + confidence, and you Confirm or Cancel. There's **no way to correct a wrong mapping or resolve an ambiguous column in-app** (`confirm=true` re-infers and applies the guess; the backend surfaces `ambiguities` but can't resolve them). Needed **only if you accept suppliers' own (non-template) sheets** — then make the mapper **editable** (a field dropdown per column + assign the ambiguous ones). Backend is a contained change (`apply_mapping` is already mapping-driven; the API confirm path must accept a buyer-edited mapping instead of re-inferring). |
