...
- **Absolute OR percent cost lines.** A cost line may come in as an **absolute $/case** or as a **percent applied at build**. **The buyer sets the base PER LINE (resolved)** — each percent line names **the base it applies to** (FOB, the running subtotal, or another named line) **plus its apply-order**. So construct-price is no longer a flat sum of $ parts: a percent line **multiplies its declared base** in the declared order. Each cost line therefore carries a **unit ($ vs %)** and, when %, a **base + apply-order**; the arithmetic must be deterministic and order-stable (E-39).
- **Discounts are PERCENT-of-FOB lines (resolved 2026-06-22 — settles finding A).** The full-lot and incremental discounts (`lot_discount` / `all_lot_discount`) are **percent of the FOB bid price** (base = FOB, not landed; applied before the surcharges). Rationale: the manual collects them as % (`% Discount for Full-Lot Award`, `Incremental % Discount`); suppliers express volume discounts as %; and a **% is grain-robust** — it scales with whatever FOB it multiplies (per-period or timeframe), where a fixed `$` discount would mean different things at different grains. **Engine change (E-44):** today `construct_price_from_parts` subtracts them as `$`; switch to **percent-of-FOB** and have the ingester read them as percents (no silent `$` treatment). The known-template adapter still converts any legacy `$`-stated discount. *(My call on the evidence; sponsor can override to `$` — it is supplier-facing.)*
...
- **% vs $ on discounts — ✅ RESOLVED:** discounts are **percent-of-FOB** (see the discount bullet above). E-44 switches the engine from `$`-subtraction to percent-of-FOB; the adapter converts legacy `$`.
