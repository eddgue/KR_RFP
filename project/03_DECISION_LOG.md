...
**STILL OPEN / FINDINGS to reconcile before building (kept separate so they can't corrupt live specs — `MANUAL_MODEL_FINDINGS.md`):**
- **% vs $ on discounts** — manual discounts are **percentages**; our engine subtracts them as **dollars** with **no %→$ conversion** in the ingester. Confirm our owned template's intended unit + add the conversion (or the known-template-adapter must convert). Validates D43's percent-line handling.
- **RPC as its own cost line** — surface RPC (+/- $/case, gated by "RPCs? Y/N") as a first-class D43 cost line rather than folding into All-In.
