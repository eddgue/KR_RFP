...
# The synthetic SETUP + bid-template builders moved to app.pilot.synthetic (pytest-free, so the
# deploy seed can reuse them). Aliased to the test's historical private names just below.
...
# ---------------------------------------------------------------------------
# helpers — the synthetic SETUP + bid-template builders.
# These live in `app.pilot.synthetic` (the single source of truth, pytest-free so the deploy seed
# can reuse them without pulling pytest into the runtime image — deploy/gcp/seed.py). The test keeps
# its historical private names as thin aliases so the cases below read unchanged.
# ---------------------------------------------------------------------------
_build_filled_setup = build_filled_setup
_fill_bid_template = fill_bid_template
